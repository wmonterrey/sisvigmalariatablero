...
public class ExportService {
	...
	public List<Object[]> getDatosCasos(Long desde, Long hasta,String oulevel,String ouname) {
		...
		String sqlQuery = "SELECT sisvigdb.malaria_casos.id, sisvigdb.notic_form.id_notic, sisvigdb.tbl_persona.fecha_nacimiento, sisvigdb.tbl_persona.sexo, "
				...
				+ ", viajes.viaje_id_localidad, focsamp.name AS 'Foco Muestra', focresd.name AS 'Foco Residencia', focinf.name AS 'Foco Infeccion' "
				...
				+ "LEFT JOIN (SELECT id_investigacion, viaje_id_localidad, COUNT(id_investigacion) AS cuenta FROM sisvigdb.malaria_investigacion_viajes WHERE viaje_id_localidad IS NOT null GROUP BY id_investigacion HAVING cuenta = 1) viajes ON malaria_investigaciones.id= viajes.id_investigacion "
				+ "LEFT JOIN sisvig_tablero.focilocalities focilsamp ON  sisvigdb.malaria_casos.pdr_muestra_id_localidad = focilsamp.locality LEFT JOIN sisvig_tablero.foci focsamp ON focilsamp.foco=focsamp.id "
				+ "LEFT JOIN sisvig_tablero.focilocalities focilresd ON sisvigdb.tbl_persona.id_localidad = focilresd.locality LEFT JOIN sisvig_tablero.foci focresd ON focilresd.foco=focresd.id "
				+ "LEFT JOIN sisvig_tablero.focilocalities focilinf ON if(origen_infeccion='residencia',sisvigdb.tbl_persona.id_localidad,viajes.viaje_id_localidad) = focilinf.locality LEFT JOIN sisvig_tablero.foci focinf ON focilinf.foco=focinf.id) "
